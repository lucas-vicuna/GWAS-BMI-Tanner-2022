...
df1f <- read_csv('bmi/two_stage_weibull/data/girls_bmi.csv')
df1m <- read_csv('bmi/two_stage_weibull/data/boys_bmi.csv')
...
dfm <- df1m %>% left_join(df3)
dff <- df1f %>% left_join(df3)
...
#   write_csv('tanner12/output_conjunto4/summary_ga_fasoc.csv')
