...
f1 <- 'tanner24/data/Tanner_24.tsv'
f2 <- paste0('bmi/detail/chr',chr,'_SNP_selected.tsv')
f3 <- paste0('/data/genetica/tomas_localancestry/gocs_pel_ceu_pel95_phen904/rfmix15/rfmix15.chr',chr,'.stats.phen.pel_ceu.txt')
f4 <- paste0('tanner24/output/chr',chr,'_tanner24.txt')
f5 <- paste0('tanner24/output/chr',chr,'_tanner24failed.txt')
...
myformula <- function(i, seg){
  ...
  p1 = paste0('cbind(Age_LastT2 , Age_FirstT4) ~ ga + ', names(df3)[5+i+k])
  p2 = ifelse(seg == 'All',
