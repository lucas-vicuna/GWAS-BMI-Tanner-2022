pdf("/data/lucas/tanner/figures/manhattans_survival_MF_tanner23.pdf",  
    width=6, height=5.5)
...
chrs=fread("/data/genetica/Tanner/tanner23_boys_survival.csv",header=T)
colnames(chrs)=c("CHR","SNP","Beta","std_error","Z_val","P_val")
snps=fread("/data/genetica/Tanner/snps_filtrados_tanner23_boys_survival.csv",
           header=T)
...
mtext(side = 3, text = expression(italic("SOX2-OT")), 
      line = 1, adj = 0.25, padj = 4.2, las = 1, cex=0.7, col="deepskyblue4") 
...
chrs=fread("/data/genetica/Tanner/tanner23_girls_survival.csv",header=T)
colnames(chrs)=c("CHR","SNP","Beta","std_error","Z_val","P_val")
snps=fread("/data/genetica/Tanner/snps_filtrados_tanner23_girls_survival.csv",
           header=T)
chrs = semi_join(chrs,snps,c("CHR","SNP"))
